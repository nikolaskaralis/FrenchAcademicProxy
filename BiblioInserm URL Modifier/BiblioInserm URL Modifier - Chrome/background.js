...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.url) {
    const modifiedUrl = modifyURL(changeInfo.url);
    if (modifiedUrl !== changeInfo.url) {
...
function modifyURL(url) {
  ...
  for (const domainToModify of domainsToModify) {
    if (domainParts.length > 2 && domainParts.slice(-2).join('.') === domainToModify) {
      const modifiedDomain = `${domainParts.slice(0, -2).join('-')}-${domainParts.slice(-2).join('-')}.proxy.insermbiblio.inist.fr`;
